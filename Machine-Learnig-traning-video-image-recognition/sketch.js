...
function videoReady(){
  ...
  extratorCaracteristiques = ml5.featureExtractor('MobileNet',options, modelLoaded);
}

function modelLoaded(){
  console.log("Model Loaded!!")
  classificador = extratorCaracteristiques.classification(video, classificadorReady);
}
...
function creacioInterficie(x,y){
 /* button = createButton('add Imatge');
  ...
  addEtiquetaImatge = createButton('add Imatge').position(x,y).mousePressed(addImatge);
 
  /*inputEtiqueta = createInput('');
  inputEtiqueta.position(20, 60);
  inputEtiqueta.size(100);*/
  inputEtiqueta = createInput('').position(x+addEtiquetaImatge.size()["width"], y).size(100);
  
  btnEntrenar = createButton("Entrenar").position(x+addEtiquetaImatge.size()["width"] + inputEtiqueta.size()["width"] ,y).mousePressed(entrenar);
